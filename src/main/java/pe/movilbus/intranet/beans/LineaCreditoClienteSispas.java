package pe.movilbus.intranet.beans;

import java.util.Date;

public class LineaCreditoClienteSispas extends GenericBean {
	private static final long serialVersionUID = 1L;
	
	private Long Id;
	private CarteraClienteSispas carteraCliente;
	private TipoCobranzaSispas tipoCobranza;
	private SolicitudClienteCreditoSispas solicitudClienteCredito;
	private Double lineaCreditoSolicitada;
	private Double lineaCreditoAprobada;
	private Double sobregiro;
	private Double saldo;
	private Date fechaActivacion;
	private Date fechaCaducidad;
	private Date fechaInactivacion;
	private Date fechaSuspension;
	private String estadoLineaCredito;
	private String esCanje;
	private String esComisionable;
	private String motivo;
	
	
	/*NO MAPEADOS*/
	private String tipoCliente;
	
	public Long getId() {
		return Id;
	}
	public void setId(Long id) {
		Id = id;
	}
	public CarteraClienteSispas getCarteraCliente() {
		return carteraCliente;
	}
	public void setCarteraCliente(CarteraClienteSispas carteraCliente) {
		this.carteraCliente = carteraCliente;
	}
	public TipoCobranzaSispas getTipoCobranza() {
		return tipoCobranza;
	}
	public void setTipoCobranza(TipoCobranzaSispas tipoCobranza) {
		this.tipoCobranza = tipoCobranza;
	}
	public SolicitudClienteCreditoSispas getSolicitudClienteCredito() {
		return solicitudClienteCredito;
	}
	public void setSolicitudClienteCredito(SolicitudClienteCreditoSispas solicitudClienteCredito) {
		this.solicitudClienteCredito = solicitudClienteCredito;
	}
	public Double getLineaCreditoSolicitada() {
		return lineaCreditoSolicitada;
	}
	public void setLineaCreditoSolicitada(Double lineaCreditoSolicitada) {
		this.lineaCreditoSolicitada = lineaCreditoSolicitada;
	}
	public Double getLineaCreditoAprobada() {
		return lineaCreditoAprobada;
	}
	public void setLineaCreditoAprobada(Double lineaCreditoAprobada) {
		this.lineaCreditoAprobada = lineaCreditoAprobada;
	}
	public Double getSobregiro() {
		return sobregiro;
	}
	public void setSobregiro(Double sobregiro) {
		this.sobregiro = sobregiro;
	}
	public Double getSaldo() {
		return saldo;
	}
	public void setSaldo(Double saldo) {
		this.saldo = saldo;
	}
	public Date getFechaActivacion() {
		return fechaActivacion;
	}
	public void setFechaActivacion(Date fechaActivacion) {
		this.fechaActivacion = fechaActivacion;
	}
	public Date getFechaCaducidad() {
		return fechaCaducidad;
	}
	public void setFechaCaducidad(Date fechaCaducidad) {
		this.fechaCaducidad = fechaCaducidad;
	}
	public Date getFechaInactivacion() {
		return fechaInactivacion;
	}
	public void setFechaInactivacion(Date fechaInactivacion) {
		this.fechaInactivacion = fechaInactivacion;
	}
	public Date getFechaSuspension() {
		return fechaSuspension;
	}
	public void setFechaSuspension(Date fechaSuspension) {
		this.fechaSuspension = fechaSuspension;
	}
	public String getEstadoLineaCredito() {
		return estadoLineaCredito;
	}
	public void setEstadoLineaCredito(String estadoLineaCredito) {
		this.estadoLineaCredito = estadoLineaCredito;
	}
	public String getEsCanje() {
		return esCanje;
	}
	public void setEsCanje(String esCanje) {
		this.esCanje = esCanje;
	}
	public String getEsComisionable() {
		return esComisionable;
	}
	public void setEsComisionable(String esComisionable) {
		this.esComisionable = esComisionable;
	}
	/**
	 * @return the tipoCliente
	 */
	public String getTipoCliente() {
		return tipoCliente;
	}
	/**
	 * @param tipoCliente the tipoCliente to set
	 */
	public void setTipoCliente(String tipoCliente) {
		this.tipoCliente = tipoCliente;
	}
	/**
	 * @return the motivo
	 */
	public String getMotivo() {
		return motivo;
	}
	/**
	 * @param motivo the motivo to set
	 */
	public void setMotivo(String motivo) {
		this.motivo = motivo;
	}
	
}
